            ...
            <div class="kt-portlet__body kt-portlet__body--fit" id="app">
                <div class="kt-grid kt-wizard-v1 kt-wizard-v1--white" id="kt_wizard_v1" data-ktwizard-state="step-first">
                    ...
                    <div class="kt-grid__item kt-grid__item--fluid kt-wizard-v1__wrapper">
                        ...
                        <form method="POST" action="{{ route('campaigns.update', $campaign->id) }}" class="kt-form" id="kt_form" enctype="multipart/form-data" data-redirect="{{ route('campaigns.index') }}">
                        @csrf
                        @method('PUT')
    ...
    <script type="text/javascript">
        let app = new Vue({
            ...
            data: {
                ...
                selected_contacts: @json($campaign->contacts),
                search: '',

                name: '{{ old('name') ?? $campaign->name }}',
                from: '{{ old('from') ?? $campaign->from }}',
                subject: '{{ old('subject') ?? $campaign->subject }}',
                template: '{{ old('template') ?? $campaign->template_id }}',
                title: '{{ old('title') ?? $campaign->title }}',
                text: '{{ old('text') ?? $campaign->text }}',
                button_text: '{{ old('button_text') ?? $campaign->button_text }}',
                button_color: '{{ old('button_color') ?? $campaign->button_color }}'
            },
            computed: {
